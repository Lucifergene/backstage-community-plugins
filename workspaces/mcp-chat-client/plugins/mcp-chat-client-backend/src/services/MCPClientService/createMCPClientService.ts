...
import { ServerConfig, MCPClientService } from './types';
...
export async function createMCPClientService({
  logger,
  config,
}: {
  ...
  logger.info('Initializing MCPClientService');

  let llmProvider: LLMProvider;

  try {
    const providerConfig = getConfig(config);
    llmProvider = ProviderFactory.createProvider(providerConfig);
    console.log(
  ...
  return {
    ...
    async processQuery(
      ...
      const llmTools: Tool[] = filteredTools.map(
        ({ serverId, ...tool }) => tool,
      );

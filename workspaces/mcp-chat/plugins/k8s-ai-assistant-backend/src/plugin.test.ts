...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [k8SAiAssistantPlugin],
    });

    await request(server).get('/api/k8s-ai-assistant/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/k8s-ai-assistant/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/k8s-ai-assistant/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/k8s-ai-assistant/todos/${createdTodoItem.id}`)
